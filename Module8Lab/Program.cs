using System;
using System.Collections.Generic;
using System.Linq;

// Pokeman class - a blueprint for creating Pokemon objects
public class Pokemon 
{
    // attributes - variables that hold values describing Pokemon
    public int Id {get; set;}
    public string? Name {get; set;}
    public string? Type {get; set;}
    public int Level {get; set;}
} // end Pokeman class

// Pokemon LINQ lab class contains main method - automatically called
public class PokemonLINQLab
{
    // main method
    public static void Main() {

        // sample data: list of Pokemon objects
        var pokemons = new List<Pokemon>
        {
            new Pokemon {Id = 1, Name = "Bulbasaur", Type = "Grass", Level = 5},
            new Pokemon {Id = 2, Name = "Charmander", Type = "Fire", Level = 36},
            new Pokemon {Id = 3, Name = "Squirtle", Type = "Water", Level = 10},
            new Pokemon {Id = 4, Name = "Pikachu", Type = "Electric", Level = 22},
            new Pokemon {Id = 5, Name = "Eevee", Type = "Normal", Level = 25},
            new Pokemon {Id = 6, Name = "Jigglypuff", Type = "Fairy", Level = 30}
        };

        // Linq query: find Pokemon that are ready to evolve (Level 16 for first ev)
        var readyToEvolve = from p in pokemons
                            where p.Level >= 16
                            orderby p.Level
                            select p;
        // execute the query and display the results
        Console.WriteLine("Pokemon Ready to Evolve: ");
        foreach (var p in readyToEvolve)
        {
            // output the current pokemon who is ready to evolve
            Console.WriteLine($"Name: {p.Name}, Type: {p.Type}, Level: {p.Level}");
        } // end for loop
    } // end of main method
} // end Linq lab class